...
● 2024년 제8회 빅데이터분석기사 필기
   ...
   - 정답: 
   - 해설:
         다변량분산분석은 하나 이상의 독립변수에 대해 여러 개의 종속변수를 동시에 분석하여, 집단 간 평균 차이가 유의한지를 검정합니다.
...
● 2022년 제4회 빅데이터분석기사 필기
   ...
   - 정답: 
   - 해설:
         모집단이 3개 이상일 때, 평균 차이를 검정하는 데는 분산분석(ANOVA)이 사용됩니다.

● 2021년 제2회 빅데이터분석기사 필기
   - 문제:
         일원배치 분산 분석(One-way ANOVA)에 대한 설명으로 옳은 것은?
         선택지:
           1) 독립변수와 종속변수가 각각 1개이고 모집단의 분산을 알지 못할 때 평균을 비교하기 위해 사용한다
           2) 독립변수가 2개, 종속변수가 1개일 때 사용한다
           3) 독립변수가 3개 이상, 종속변수가 1개일 때 사용한다
           4) 독립변수와 종속변수가 모두 2개 이상일 때 사용한다
   - 정답: 
   - 해설:
         일원배치 ANOVA는 하나의 요인(독립변수)과 하나의 종속변수를 대상으로, 모집단 분산을 모를 때 그룹 간 평균 차이를 검정하는 방법입니다.
